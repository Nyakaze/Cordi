...
public static class GameFontIcons
{
    private const string AtlasPath = "common/font/fonticon_ps5.tex";
    private const float AtlasRowOffset = 2f;
        ...
        out Vector2 uv1)
    {
        ...
        var bounds = Bounds(id);
        if (bounds == null) return false;

        var wrap = Service.TextureProvider.GetFromGame(AtlasPath).GetWrapOrDefault();
        if (wrap == null || wrap.Width <= 0 || wrap.Height <= 0) return false;

        var box = bounds.Value;
        if (box.W <= 0f) return false;

        var atlas = new Vector2(wrap.Width, wrap.Height);
        var ratio = lineHeight / box.W;

        texture = wrap;
        size = new Vector2(box.Z, box.W) * ratio;
        uv0 = new Vector2(box.X, box.Y - AtlasRowOffset) / atlas;
        uv1 = new Vector2(box.X + box.Z, box.Y - AtlasRowOffset + box.W) / atlas;
        return true;
    }

    private static Vector4? Bounds(uint id) => id switch
    {
